        ...
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div><label className={labelStyle}>Mins</label><input type="number" className={inputStyle} value={formData.minutes_played} onChange={e => setFormData({...formData, minutes_played: e.target.value})} /></div>
                    <div><label className={labelStyle}>Rating</label><input type="number" step="0.1" className={inputStyle} placeholder="8.5" required onChange={e => setFormData({...formData, rating: e.target.value})} /></div>
                    <div><label className={labelStyle}>Goals</label><input type="number" className={inputStyle} value={formData.goals} onChange={e => setFormData({...formData, goals: e.target.value})} /></div>
                    <div><label className={labelStyle}>Assists</label><input type="number" className={inputStyle} value={formData.assists} onChange={e => setFormData({...formData, assists: e.target.value})} /></div>
                </div>
